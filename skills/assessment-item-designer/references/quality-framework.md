...
Use `review_mode: standard` unless the instructor explicitly requests `high_assurance`. In standard MCQ review, use two distinct fresh subagents: a target-blind cognitive classifier and a key-blind independent item judge. Add one fresh key-blind tie-break reviewer only when a documented uncertainty or disagreement trigger exists. In high-assurance MCQ review, retain four fresh subagents (classification, answer solver 1, answer solver 2, final judge). Essays use a fresh classifier and a fresh independent final/scoring judge; do not add reviewers merely to mirror MCQ counts. Start every review with no inherited conversation history; with Codex collaboration tools use `spawn_agent` with `fork_turns: "none"`. Here history means earlier task dialogue, not baseline system/developer instructions or profile/environment context supplied by the host. If that baseline itself exposes prohibited assessment information, isolation is unverified. Never reuse a reviewer for another role, candidate, or revision. Discard earlier reviews after an item changes and review the revised item with new subagents.
...
- Classification: item prompt/stem, options when present, and permitted resources only. No answer outline, rubric, keys, rationales, exemplar memory, target labels, generator metadata, or prior results. Return Bloom, estimated difficulty, confidence and a concise basis before the coordinator compares them with targets. Approved historical or calibration items may inform difficulty after Bloom is fixed; redact keys and target labels for the current candidate and do not reveal run-exemplar verdicts. Cite comparator source locators in the difficulty basis; comparators never expand scope.
- Standard item judge: stem, stable option IDs and text, permitted resources, relevant authorized grounding evidence, learning outcome, assessed concepts/scope, points and other authorized blueprint constraints. It independently solves the item, identifies a defensible `option_id`, and records the option-by-option and whole-set quality review below, plus one-best-answer status, ambiguity, fairness, construct relevance and alignment. Remove generated keys, answer rationales, misconception rationales, target labels, exemplar memory, prior results and revision history.
- Tie-break reviewer: stem, stable option IDs and text, and permitted resources only; reorder options where possible. It is invoked only for a documented escalation trigger and must not see the key, prior selections/verdicts, rationales, target metadata, revision history or exemplar memory.
- High-assurance answer solvers: stem, stable option IDs and text, and permitted resources only. Solver 2 receives an order different from both the displayed original and solver 1. Return the chosen option ID and a short answer justification.
- Final judge: item, permitted resources, relevant authorized grounding evidence, and the approved position's scope, outcome, points and constraints. Remove target Bloom/difficulty labels and all keys, answer outlines, rubrics, rationales, exemplar memory and earlier results. For high-assurance MCQs, record the same option-by-option and whole-set quality review as the standard item judge. For essays, independently establish scoring expectations; the coordinator then checks the generated outline and rubric against those expectations.

Run each role without revealing prohibited results from preceding reviews. Reviews may run sequentially to respect agent limits; dispose of completed reviewer sessions when the runtime supports it. Never weaken isolation to work around capacity limits. Preserve the generate–judge–refresh sequence between candidates.
...
If a mandatory call cannot run or completes with unverified isolation, retain an unresolved escalation and a non-passing candidate. Do not fabricate an agent ID or result. Resume using new subagents when available. High-assurance mode must not fall back to standard. Instructor approval never waives mandatory review; manual review remains available for substantive issues such as flagged lexical similarity.

The additional reviewer in high-assurance mode provides procedural assurance, not an empirically established psychometric advantage. Independent LLM agreement is not post-administration validation.
...
Any `fail` or `uncertain` result blocks automated `pass`. Revise and obtain fresh independent reviews, reject, or retain unresolved manual review as appropriate. A fresh answer tie-break may resolve a selection disagreement, but cannot waive a flagged option-quality problem. The audit validator can enforce the presence, completeness and disposition of these judgments; it cannot verify their semantic accuracy.
